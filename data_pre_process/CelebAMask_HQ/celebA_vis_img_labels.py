...
colors = torch.as_tensor([i for i in range(7)])[:, None] * palette
...
if __name__ == '__main__':
    ...
    for file in os.listdir(img_path):
        if os.path.exists(os.path.join(lbl_path, file[0:-4] + ".png")) is False:
            continue
        print(file)
        image = Image.open(os.path.join(img_path, file)).convert('RGB').resize((512, 512))
        ...
        vutils.save_image(label_show, os.path.join("./test", "vis_%s" % file))
